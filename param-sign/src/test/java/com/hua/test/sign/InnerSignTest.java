...
@ContextConfiguration(locations = {
		...
		"classpath:conf/xml/spring-interceptor.xml"
		})
@RunWith(SpringJUnit4ClassRunner.class)
public final class InnerSignTest extends BaseTest {
	...
	public void testAndroidClient() {
		try {
			 ...
			 * 设置请求参数
			 */
			final String token = "AKDSKWE123SD2334";
			final String t = String.valueOf(new Date().getTime());
			final String r = getRandomNumber(10);
			final String appKey = "a_123";
			final String apiSecret = "api_20160125";
			final String userSecret = "u_123456";
			final String ct = "0";
			final String appName = "app_name_t";
			final String m = "m_1238172";
			final String v = "1";
			requestBuilder.param("username", "admin");
			requestBuilder.param("tk", token);
			requestBuilder.param("ct", ct);
			requestBuilder.param("t", t);
			requestBuilder.param("app", appName);
			requestBuilder.param("v", v);
			requestBuilder.param("m", m);
			requestBuilder.param("r", r);
			RequestParamsHelper helper = new RequestParamsHelper();
			helper.addParameter("ct", ct);
			// appKey
			helper.addParameter("ak", appKey);
			helper.addParameter("t", t);
			helper.addParameter("app", appName);
			helper.addParameter("v", v);
			helper.addParameter("m", m);
			helper.addParameter("r", r);
			helper.setApiSecret(apiSecret);
			helper.setUserSecret(userSecret);
			String s = null;
			// client: {app=app_name_t, ct=0, r=2631206985, t=1453776640192, v=1, ak=a_123, m=m_1238172}
			// server: {app=app_name_t, ct=0, r=2631206985, t=1453776640192, v=1, ak=a_123, m=m_1238172}
			// 生成签名
			s = helper.removeParameter("s")
					.removeParameter("tk").sort().getSortedValuesAsSha1();
			
			requestBuilder.param("s", s);
			// 模拟 mvc 对象，设置 WebApplicationContext，然后构建 模拟mvc对象
			MockMvc mockMvc = MockMvcBuilders.webAppContextSetup(webApplicationContext).build(); 
			// mvc 结果
			MvcResult mvcResult = mockMvc.perform(requestBuilder).andReturn();
	...
	public void testIOSClient() {
		try {
			...
			final String t = String.valueOf(new Date().getTime());
			final String r = getRandomNumber(10);
			final String appKey = "i_123";
			final String apiSecret = "api_20160125";
			final String userSecret = "u_123456";
			final String ct = "1";
			final String appName = "app_name_t";
			final String m = "uuid_1238172";
			final String v = "2";
			requestBuilder.param("username", "admin");
			requestBuilder.param("tk", token);
			requestBuilder.param("ct", ct);
